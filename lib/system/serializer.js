...
Serializer.prototype._serialize = function (obj, cache, path)
{
    if (typeof obj === "object")
    {
        ...
        {
            outputObj = serializeToJSON.call(obj);
            if (!_.isObject(outputObj)) throw new Error(obj.constructor.name + ".serializeToJSON result not an object.");
...
Serializer.prototype._unserialize = function (obj, originObj, circularTasks)
{
    if (typeof obj === "object")
    {
        ...
        if (obj !== null && obj[Serializer.TOJSONTAG])
        {
            result = {};
            var typeTag = obj[Serializer.TYPETAG];
            if (typeTag !== undefined) result[Serializer.TYPETAG] = typeTag;
        }
        else
        {
            for (var key in obj)
        ...
        if (result !== null)
        {
            ...
            if (obj[Serializer.TOJSONTAG])
            {
                var deserializeFromJSON = result["deserializeFromJSON"] || result["_deserializeFromJSON"];
                if (_.isFunction(deserializeFromJSON))
                {
                    delete obj[Serializer.TOJSONTAG];
                    deserializeFromJSON.call(result, obj);
                }
                else
                {
